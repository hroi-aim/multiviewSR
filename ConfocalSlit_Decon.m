...
PreProcessing = 0; StepA = 1; StepB = 1; StepC=0.5; % step size for each view
Scanning_Mode = 1; % 1 stage; 0 OBJ
RegFlag = 1; % 
CoarseRegFlag = 0;
RegAllTimePoints = 0; % 1 for all time points; 0 for one matrix to apply all time points. 
TripleViewDeconFlag = 0; % 1 for triple-view decon; 0 for not to run decon -> 
DualViewDeconFlag = 0; % 1 for dual-view decon
SingleViewDeconFlag = 0;% 1 for viewcC - decon; 0 for not
...
if SIM_Mode == 1
    ...
    PSFA = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_A_SIM_48nm.tif'));
    PSFB = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_B_SIM_48nm.tif'));
    PSFC = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_C_SIM_48nm.tif'));
    PSFA_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_A_SIM_48nm_bp.tif'));
    PSFB_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_B_SIM_48nm_bp.tif'));
    PSFC_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_C_SIM_48nm_bp.tif'));
else 
    PSFA = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_A_DL_97nm.tif'));
    PSFB = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_B_DL_97nm.tif'));
    PSFC = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_C_DL_97nm.tif'));
    PSFA_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_A_DL_97nm_bp.tif'));
    PSFB_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_B_DL_97nm_bp.tif'));
    PSFC_bp = single(ImageJ_formatted_TIFF.ReadTifStack('H:\TripleViewConfocal\NewPSF20200804\PSF_C_DL_97nm_bp.tif'));
end
...
viewA = max(single(ImageJ_formatted_TIFF.ReadTifStack([file_path, filenameA])),0);
viewB = max(single(ImageJ_formatted_TIFF.ReadTifStack([file_path, filenameB])),0);
viewC = max(single(ImageJ_formatted_TIFF.ReadTifStack([file_path, filenameC])),0); 
...
%     viewC = viewC(cropCy:cropCy+sizeCy-1,cropCx:cropCx+sizeCx-1,:);
   [viewC,cropBoxC] = Confocal_PreProcessing_PTZMode(viewC, 'ViewC', StepC, SIM_Mode, cropModeC, cropBoxC);
   ...
   elseif Scanning_Mode == 0  % moving OBJ PZT 
        viewA = Confocal_PreProcessing_PTZMode(viewA, 'ViewA', StepA, SIM_Mode, cropModeA);
      %  ImageJ_formatted_TIFF.WriteTifStack(viewA,[file_path, '\Processing\ViewA\Raw_', filenameA(1:end-4), '.tif'],'16');
        viewB = Confocal_PreProcessing_PTZMode(viewB, 'ViewB', StepB, SIM_Mode, cropModeB); 
      %  ImageJ_formatted_TIFF.WriteTifStack(viewB,[file_path, '\Processing\ViewB\Raw_', filenameB(1:end-4), '.tif'],'16');
   end
    ...
    tic
       %stitching ViewA
        TileMerge = zeros(nyC,nxC, nzC);
         ...
         ImageJ_formatted_TIFF.WriteTifStack(TileMerge,[file_path, '\Processing\Reg_', filenameA],'16'); 
       ...
       %stitching ViewB
        TileMerge = zeros(nyC,nxC, nzC);
         ...
         ImageJ_formatted_TIFF.WriteTifStack(TileMerge,[file_path, '\Processing\Reg_', filenameB],'16'); 
...
if TripleViewDeconFlag == 1
       %%stitching triple-view decon
        ...
        ImageJ_formatted_TIFF.WriteTifStack(TileMerge,[file_path, '\Processing\AdditiveDeconViewABC\TripleViewDecon_C', num2str(color(c)),'_t', num2str(time(t)),'.tif'],'32');
end
...
if DualViewDeconFlag == 1
       TileMerge = zeros(nyC,nxC, nzC);
        ...
        ImageJ_formatted_TIFF.WriteTifStack(TileMerge,[file_path, '\Processing\DualViewDecon_', filenameC(1:end-4), '.tif'],'16'); 
end
...
if SingleViewDeconFlag == 1   
        ...
        ImageJ_formatted_TIFF.WriteTifStack(TileMerge,[file_path, '\Processing\DeconViewC\SingleViewDecon_C', num2str(color(c)),'_t', num2str(time(t)),'.tif'],'32'); 
end
